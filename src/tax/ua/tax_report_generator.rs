...
impl UaTaxReportGenerator {
    ...
    pub fn get_unformal_tax_report(
        &self,
    ) -> Result<UaInvestmentTaxReport, UaTaxReportGeneratorError> {
        ...
        let currencies = broker_report
            .trades
            .iter()
            .map(|trade| trade.currency)
            .collect::<HashSet<Currency>>();

        let preserved_rated_by_currency =
            self.preserve_convert_rate_by_currencies(currencies, earliest_date, latest_date)?;
        ...
        for trade in &broker_report.trades {
            ...
            let buy_price_uah = self
                .convert_to_uah_from_preserved_by_currency(
                    buy_price,
                    currency,
                    trade.buy_date,
                    &preserved_rated_by_currency,
                )?
                .round_dp(2);
            let sell_price_uah = self
                .convert_to_uah_from_preserved_by_currency(
                    sell_price,
                    currency,
                    trade.sell_date,
            ...
            investment_tax_report.investment_ops.total_tax =
                investment_tax_report.investment_ops.total_tax
                    + UaTax::new(income_tax, military_tax);
        }
        investment_tax_report.investment_ops.total_tax =
            investment_tax_report.investment_ops.total_tax.round_dp(2);
        ...
        for dividend in broker_report.dividends {
            ...
            println!(
                "Dividend for {}: {} {}, {} UAH",
                dividend.name, amount, currency, amount_uah
            );
            ...
            investment_tax_report.dividend_ops.total_tax =
                investment_tax_report.dividend_ops.total_tax + UaTax::new(income_tax, military_tax);
        }
        investment_tax_report.dividend_ops.total_tax =
            investment_tax_report.dividend_ops.total_tax.round_dp(2);

        investment_tax_report.dividend_ops.income_total =
            investment_tax_report.dividend_ops.income_total.round_dp(2);

        Ok(investment_tax_report)
    }

    fn get_total_range_by_report(
        &self,
        broker_report: &BrokerReport,
    ) -> Result<(NaiveDate, NaiveDate), UaTaxReportGeneratorError> {
        let min_date_trades = broker_report
            .trades
            .iter()
            .map(|trade| trade.buy_date)
            .min();
        let min_date_dividends = broker_report
            .dividends
            .iter()
            .map(|dividend| dividend.date)
            .min();

        let max_date_trades = broker_report
            .trades
            .iter()
            .map(|trade| trade.sell_date)
            .max();
        let max_date_dividends = broker_report
            .dividends
            .iter()
            .map(|dividend| dividend.date)
            .max();
    ...
    fn preserve_convert_rate_by_currencies(
        &self,
        currencies: HashSet<Currency>,
        start_date: NaiveDate,
        end_date: NaiveDate,
    ) -> Result<HashMap<Currency, Vec<CurrencyRate>>, UaTaxReportGeneratorError> {
        let mut preserved_rated_by_currency: HashMap<Currency, Vec<CurrencyRate>> = HashMap::new();
        for currency in currencies {
            let preserved_rates = self
                .preserve_convert_rate(currency, start_date, end_date)
                .unwrap();
    ...
    fn preserve_convert_rate(
        &self,
        currency: Currency,
        start_date: NaiveDate,
        end_date: NaiveDate,
    ) -> Result<Vec<CurrencyRate>, UaTaxReportGeneratorError> {
        let convert_rates = self.national_bank_rate_provider.convert_range(
            currency,
            Currency::UAH,
            start_date,
            end_date,
        ...
        match convert_rates {
            ...
            Err(e) => {
                ...
                Err(UaTaxReportGeneratorError::CurrencyConvertorError(
                    "Error preserving currency rates".to_string(),
                ))
            }
        }
    }

    fn convert_to_uah_from_preserved_by_currency(
        &self,
        amount: Decimal,
        currency: Currency,
        date: NaiveDate,
        preserved_rates: &HashMap<Currency, Vec<CurrencyRate>>,
    ) -> Result<Decimal, UaTaxReportGeneratorError> {
        ...
        match rates {
            Some(rates) => {
                ...
                match rate {
                    ...
                    None => Err(UaTaxReportGeneratorError::CurrencyConvertorError(format!(
                        "Could not find rate for currency {} at date {}",
                        currency, date
                    ))),
                }
            }
            None => Err(UaTaxReportGeneratorError::CurrencyConvertorError(
                "Could not get preserved rates".to_string(),
            )),
        }
    }

    fn tax_policy_by_date(
        &self,
        date: NaiveDate,
